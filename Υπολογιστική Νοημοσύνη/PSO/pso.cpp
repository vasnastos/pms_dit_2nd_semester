...
PSO::PSO(Problem *p,int number_of_iterations,int particle_count):problem(p),max_iters(number_of_iterations),particle_count(particle_count),iter(0),inertia_max(0.999),inertia_min(0.4) {
    ...
    // this->inertia.resize(particle_count);
    // fill(this->inertia.begin(),this->inertia.end(),0.7);
}
...
void PSO::step()
{
    this->iter++;
    double r1,r2,r3;
    int problem_dimension=this->problem->getDimension();
    uniform_real_distribution <double> rand_real_eng(0,1);
    double y,velocity_y,by,adaptive_value;
    ...
    // this->inertia=this->inertia_max-((static_cast<double>(this->iter)/static_cast<double>(this->max_iters))*(this->inertia_max-this->inertia_min));
    ...
    {   
        ...
        if(!this->problem->isPointInside(x)) continue;

        y=this->problem->statFunmin(x);
        this->particle.replace_point(i,x,y);

        // Adaptive inertia
        // adaptive_value=(this->best_y-y)/(this->best_y+y);
        // this->inertia[i]=(this->inertia_min)+(this->inertia_max-this->inertia_min)*((exp(adaptive_value)-1)/exp(adaptive_value)+1);
...
{
    Data geometric_center_points;
    geometric_center_points.resize(this->problem->getDimension());
    ...
    for(int j=0,t=this->problem->getDimension();j<t;j++)
    {
